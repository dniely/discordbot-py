...
TOKEN = 'MTQ4MDk0MzQ3NzgwOTM0ODYxOQ.GxG2YT.CuJSwokBqi67rjsUZTHyi4_Hx6ziREox6z-5pY'
...
tier_code_map = {
    ...
    "d1": 25, "d2": 24, "d3": 23, "d4": 22, "d5": 21,
    "r1": 30, "r2": 29, "r3": 28, "r4": 27, "r5": 26
}

tag_list = [
    "implementation", "greedy", "dp", "math", "graphs", "bfs", "binary_search", "string", "bruteforcing",
    "data_structures", "segment_tree", "trees", "sorting", "dfs", "dijkstra", "shortest_path",
    "prefix_sum", "two_pointer", "stack", "queue", "deque", "priority_queue", "hash_map",
    "union_find", "topological_sorting", "bellman_ford", "floyd_warshall", "mst",
    "bitmask", "backtracking", "divide_and_conquer", "number_theory", "combinatorics",
    "geometry", "simulation", "recursion", "sparse_table", "monotone_stack",
    "sliding_window", "trie", "knapsack", "lis", "lcs", "flow", "bipartite_matching",
    "euler_tour", "hld", "sqrt_decomposition", "offline_queries", "convex_hull"
]
...
async def 등록(ctx, handle=None):
    ...
    res = requests.get("https://solved.ac/api/v3/user/show?handle=" + handle, timeout=5)
    data = res.json()
...
async def 내정보(ctx):
    ...
    res = requests.get("https://solved.ac/api/v3/user/show?handle=" + bj_id, timeout=5)
    data = res.json()
...
async def 추천(ctx):
    ...
    url = "https://solved.ac/api/v3/search/problem?query=tier:" + str(tn) + " " + tag_q + " !solved_by:" + bj + "&sort=random&direction=desc"
...
async def 쉬운문제(ctx):
    ...
    url = "https://solved.ac/api/v3/search/problem?query=tier:" + str(low) + ".." + str(high) + " !solved_by:" + bj_id + "&sort=random&direction=desc"
...
async def send_probs(ctx, data, title):
    ...
    for p in items:
        ...
        msg = msg + "https://www.acmicpc.net/problem/" + str(p["problemId"]) + " | 맞은 사람: " + str(solved_count) + "명\n"
        msg = msg + "\n"
        count = count + 1
...
async def 시작(ctx):
    ...
    try:
        ...
        await ctx.send(ctx.author.mention + " DM을 열어두셔야 도움말을 받을 수 있어요!", delete_after=5)
